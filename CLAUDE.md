...
这是一个基于R Shiny开发的专业心理量表网络分析应用，支持多种高级网络分析方法和完整的可重现研究流程。经过大量优化，现已成为功能完整、技术健壮的心理网络分析工具。

### 🏗️ 应用规模与架构 
- **代码规模**：超过11,000行专业R代码（app.R: 8,300+行，utils.R: 3,400+行）
- **支持量表**：8+种核心心理量表（AUDIT、HRF、PHQ9、GAD7、BDI、DASS、IAT、FTND等）
- **分析模块**：7种主要分析类型（基础网络、桥接分析、稳定性分析、贝叶斯网络、温度分析、组别对比、估计方法选择）
- **UI页面**：6个功能完整的分析页面，专门的变量选择系统
- **代码导出**：完整的R脚本自动生成，记录所有实际执行的分析步骤

### 🌟 核心特性
- **🎛️ 多网络估计方法支持**：EBICglasso（推荐）、MGM（混合数据）、Ising（二元数据）、相关网络、偏相关、TMFG等，含详细使用建议
...
```
NetworkApp/
├── app.R                    # 主应用文件（7,975行，400KB+）
├── utils.R                  # 工具函数库（2,560行，30+函数）
├── config.R                 # 配置文件（71种量表定义、参数设置）
├── 安全启动.R               # 推荐启动脚本（避免bruceR冲突）
├── run_app.R                # 完整启动脚本
├── CLAUDE.md                # 使用指南（本文件）
├── CLAUDE_ARCHITECTURE.md   # 完整技术架构文档（11,000+行代码分析）
├── README.md                # 项目说明
├── 最终使用说明.md          # 详细使用指南
...
| 文件 | 行数 | 主要内容 | 占比 |
|------|------|----------|------|
| app.R | 7,975 | UI界面(1,500行) + 服务器逻辑(6,400行) | 74% |
| utils.R | 2,560 | 30+专业分析函数 | 24% |
| config.R | ~500 | 71种量表配置 | 2% |
| **总计** | **11,035** | **完整网络分析平台** | **100%** |
